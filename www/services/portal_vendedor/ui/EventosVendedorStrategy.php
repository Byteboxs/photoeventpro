...
class EventosVendedorStrategy
{
    ...
    public function apply(&$data)
    {
        ...
        ArrayModifierHelper::removeColumn('project_status', $data);
        ArrayModifierHelper::removeColumn('end_date', $data);
        ArrayModifierHelper::removeColumn('institution_name', $data);
        ArrayModifierHelper::removeColumn('start_date', $data);
    ...
    private function modifyProjectName($item, $args)
    {
        $projectName = ucfirst(mb_strtolower($item['project_name']));
        $institutionName = $item['institution_name'];
        $institutionNameArray = explode(' ', $institutionName);
        $isHabilitado = $item['is_habilitado'];
        $aleatorio = rand(0, 6);

        $tooltip = 'La POS esta deshabilitada para el evento ' . $projectName;

        $url = 'javascript:void(0);';
        if ($isHabilitado == 1) {
            $url = RouteHelper::getUrlFor('posView', ['evento_id' => $item['project_id']]);
            $tooltip = 'Ir a la POS para facturar el evento ' . $projectName;
        }

        $color = $this->colors[$aleatorio];
        $primerCaracter = strtoupper(str_split($institutionNameArray[0])[0]);
        $segundoCaracter = strtoupper(str_split($institutionNameArray[1])[0]);
    ...
    private function modifyIsHabilitado($item, $args)
    {

        $status = $item[$args['col']];
        $response = ($status == 1) ? ['success', 'Habilitado'] : ['danger', 'Deshabilitado'];
        return new Badge($response[1], $response[0]);
    }
