...
class UIProductosService
{
    ...
    public function getServiciosCards(array $servicios, string $type = 'disponibles')
    {
        $serviciosCards = '';
        if (count($servicios) > 0) {
            if ($type == 'disponibles') {
                foreach ($servicios as $servicio) {
                    $serviciosCards .= $this->getServicioCardDragable($servicio);
                }
            }
            if ($type == 'seleccionados') {
                foreach ($servicios as $servicio) {
                    $serviciosCards .= $this->getServicioCardPreloaded($servicio);
                }
            }
        }
        return $serviciosCards;
    }

    public function getServicioCardDragable($vendedor)
    {
    ...
    public static function getServicioCardPreloaded($vendedor)
    {
        $primerCaracter = strtoupper(str_split($vendedor->primer_nombre)[0]);
        $segundoCaracter = strtoupper(str_split($vendedor->primer_apellido)[0]);
        $avatarName = $primerCaracter . $segundoCaracter;
